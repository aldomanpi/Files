...
namespace Files.App.Utils.Storage
{
	...
	public sealed partial class FilesystemOperations : IFilesystemOperations
	{
		...
		public async Task<IStorageHistory?> MoveAsync(IStorageItemWithPath source, string destination, NameCollisionOption collision, IProgress<StatusCenterItemProgressModel>? progress, CancellationToken cancellationToken)
		{
			...
			bool movedToGeneratedName = false;
			...
			else if (source.ItemType == FilesystemItemType.File)
			{
				...
				if (!fsResult)
				{
						...
						destinationResult.Result is { } destinationFolder)
					{
						...
						// When the name is already taken the storage layer moves the item under a generated name,
						// check beforehand because that name is never reported back
						bool destinationExisted = false;
						if (collision == NameCollisionOption.GenerateUniqueName)
						{
							var existingItem = await FilesystemTasks.WrapNullable<IStorageItem>(() => destinationFolder.TryGetItemAsync(newName).AsTask());
							destinationExisted = existingItem.Result is not null;
						}
						...
						if (fsResultMove && destinationExisted)
							movedToGeneratedName = true;
			...
			// The item was moved under a name generated by the storage layer, it's unknown here so there's nothing to undo
			if (movedToGeneratedName)
				return null;
