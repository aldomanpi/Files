...
namespace Files.App.ViewModels.UserControls
{
	public sealed partial class SidebarViewModel : ObservableObject, IDisposable
	{
		...
		private async Task HandleDriveItemDragOverAsync(DriveItem driveItem, ItemDragOverEventArgs args)
		{
			...
			// An unknown capacity means the drive isn't ready, except on portable devices (\\?\ paths) which never report one
			if ((Strings.Unknown.GetLocalizedResource().Equals(driveItem.SpaceText, StringComparison.OrdinalIgnoreCase) &&
				!drivePath.StartsWith(@"\\?\", StringComparison.Ordinal)) ||
				driveItem.Type is Data.Items.DriveType.NoRootDirectory ||
